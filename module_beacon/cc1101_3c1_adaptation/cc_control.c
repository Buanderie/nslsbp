...
int rs_255_223_encode_and_send(int fd, unsigned char * buff, int size)
{
...
int main(void)
{
    ...
    int i;
    for (i = 0; i < RS_PKT_LEN - 1; i++){
        data[i] = rand()%256;
    }
    printf("Send Packet has returned: %d\n", rs_255_223_encode_and_send(spifd, data, RS_PKT_LEN - 1));
    for (i = 0; i < RS_PKT_LEN - 1; i++){
        printf("0x%02X%s" , data[i], (i % 8 == 7) ? "\n" : " ");
    }
    return 0;
}
